...
Next, create an entity that use the interface. The example below implements an AXI-Stream broadcaster because it's a short, standalone example using an interface at the input port and an array of interfaces at the output port, so it covers most of the standard use cases.
...
Alternatively, you could break out individual interfaces instead of using an array. While this partially defeats the elegance of interfaces, it’s useful when you need fine-grained control or signal-level customization.
...
architecture rtl of top is
  ...
  signal m0_axis : axis_t (
    tdata(DW - 1 downto 0),
    tkeep(KW - 1 downto 0),
    tuser(UW - 1 downto 0)
  );

  signal m1_axis : axis_t (
    tdata(DW - 1 downto 0),
    tkeep(KW - 1 downto 0),
    tuser(UW - 1 downto 0)
  );

  signal m2_axis : axis_t (
    tdata(DW - 1 downto 0),
    tkeep(KW - 1 downto 0),
    tuser(UW - 1 downto 0)
  );
...
Here's an example from the excellent [taxi repo](https://github.com/fpganinja/taxi/) of SV components:
...
In practice, I haven’t missed this capability. Unconstrained vectors handle width flexibility cleanly. Optional signals (such as `tlast` or `tuser`) are more awkward, but I prefer documenting expected behavior, having the user tie unused input pins to constants, leaving unused outputs disconnected, and allowing synthesis to optimize unused signals away.
...
- **NVC Issue #1** - Cannot assign interface elements from within a loop in a process.
...
For now, I found that the best workaround is to use an intermediate signal. For example, this works without issue.
