...
So its fairly straightforward. Its just provides an axi-lite access interface to the build-time settable values. As a small bonus, I also included a scratchpad register in `stdver_axil`, which can help with verifying register read/write access in a standardized way during initial board bring-up.
